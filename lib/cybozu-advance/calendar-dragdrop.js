...
CBLabs.CybozuAdvance.CalendarDragDrop = {
    ...
    moveOrResizeEvent: function (calendar, event, dayDelta, minuteDelta, allDay, revertFunc, unselectFunc, resize) {
        ...
        if (event.event_type == "repeat") {
            ...
            if (!resize && event.event_type == "banner" && !allDay) {
                alert("期間予定を通常予定に変更することはできません。");
                unselectFunc();
                revertFunc();
                return;
            }
    ...
    moveOrResizeNormalEvent: function (calendar, event, revertFunc, unselectFunc, resize, confirm) {
        ...
        if (result) {
            if (resize) {
                if (event.start_only && event.start && event.end) {
                    event.start_only = false;
                } else if (!event.start_only && event.start && !event.end) {
                    event.start_only = true;
                }
            } else {
                var allDayClass = "event-allday";
                if ((event.allDay && event.className != allDayClass) || (!event.allDay && event.className == allDayClass)) {
                    event.className = event.allDay ? allDayClass : "";
                    $("#" + calendar.PersonalCalendarId()).fullCalendar("renderEvent", event);
                }
            }
        } else {
            revertFunc();
